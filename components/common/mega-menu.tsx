        ...
        {items?.map((item: any) => (
          <li key={item.id} className="group py-4">
            ...
            {item.children.length > 0 && (
              <div className="bg-white absolute z-999 left-0  top-32 hidden group-hover:block w-screen min-h-[30vh] pt-2 shadow-sm">
                        ...
                        <ul className="space-y-2 py-4">
                          {child.children.map((subChild: any) => (
                            <li key={subChild.id}>
                              <Link
                                href={subChild.url}
                                className="text-md text-gray-600 hover:text-primary/80 transition-colors block"
                              >
                                {subChild.label}
                              </Link>
                            </li>
